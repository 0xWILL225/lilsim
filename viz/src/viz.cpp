...
Application::Application(scene::SceneDB& db, sim::Simulator& sim, const std::filesystem::path& install_root)
  ...
  try {
    m_install_root = install_root.empty()
                      ...
                      : (install_root.is_absolute() ? install_root : std::filesystem::absolute(install_root));
  } catch (...) {
    m_install_root = std::filesystem::current_path();
  }

  std::string default_track_path = (m_install_root / "tracks").string();
  std::strncpy(m_track_dir_buffer, default_track_path.c_str(), sizeof(m_track_dir_buffer) - 1);
  m_track_dir_buffer[sizeof(m_track_dir_buffer) - 1] = '\0';

  m_viewport_panel = ViewportPanel(&m_marker_system, &m_show_car, &m_show_cones);
...
static void scrollCallback(GLFWwindow* window, double xoffset, double yoffset) {
  ...
  if (app) {
    ...
    if (app->m_viewport_panel.cameraMode == ViewportPanel::CameraMode::Free) {
      ...
      app->m_viewport_panel.freeCameraZoom = std::clamp(app->m_viewport_panel.freeCameraZoom, 5.0f, 500.0f);
    } else {
      app->m_viewport_panel.followCarZoom *= zoom_delta;
      app->m_viewport_panel.followCarZoom = std::clamp(app->m_viewport_panel.followCarZoom, 5.0f, 500.0f);
    }
  }
}
...
bool Application::initGLFW() {
  ...
  m_window = glfwCreateWindow(static_cast<int>(m_width), static_cast<int>(m_height), "lilsim", nullptr, nullptr);
...
bool Application::initWebGPU() {
  ...
  surface_desc.nextInChain = reinterpret_cast<WGPUChainedStruct const*>(&from_xlib);
...
void Application::onResize(uint32_t new_width, uint32_t new_height) {
  ...
  m_width = new_width;
  m_height = new_height;
  if (m_guiConfig.window_width != new_width || m_guiConfig.window_height != new_height) {
...
void Application::refreshAvailableModels() {
    ...
    for(size_t i=0; i<m_availableModels.size(); ++i) {
        if(!configured_path.empty() && m_availableModels[i].path == configured_path) {
...
void Application::onModelChanged() {
    ...
    m_ui_param_values.assign(desc->param_values, desc->param_values + desc->num_params);
    m_ui_setting_values.assign(desc->setting_values, desc->setting_values + desc->num_settings);
    ...
    for(size_t i=0; i<desc->num_inputs; ++i) {
        ...
        if (name == "steering_wheel_angle_input") m_input_idx_wheel_angle = i;
        else if (name == "steering_wheel_rate_input") m_input_idx_wheel_rate = i;
        else if (name == "ax" || name == "ax_input") m_input_idx_ax = i;
    }
    ...
    for(size_t i=0; i<desc->num_states; ++i) {
        ...
        if (name == "x") m_state_idx_x = i;
        else if (name == "y") m_state_idx_y = i;
        else if (name == "yaw") m_state_idx_yaw = i;
        else if (name == "v") m_state_idx_v = i;
        else if (name == "ax") m_state_idx_ax = i;
        else if (name == "steering_wheel_angle") m_state_idx_steer_wheel_angle = i;
        else if (name == "steering_wheel_rate") m_state_idx_steer_wheel_rate = i;
        else if (name == "wheel_fl_angle") m_state_idx_wheel_fl = i;
        else if (name == "wheel_fr_angle") m_state_idx_wheel_fr = i;
    }
    ...
    for(size_t i=0; i<desc->num_params; ++i) {
        ...
        if (name == "wheelbase") m_param_idx_wheelbase = i;
        else if (name == "track_width") m_param_idx_track_width = i;
    }
...
void Application::handleInput() {
  ...
  if (m_selected_model_index == common::kNullIndex && !m_availableModels.empty()) {
      onModelChanged();
  }

  const scene::Scene& scene = m_sceneDB.snapshot();
  double sim_time = static_cast<double>(m_sceneDB.tick.load(std::memory_order_relaxed)) * m_simulator.getDt();
  ViewportPanel::RenderState render_state;
  render_state.sim_time = sim_time;
  if (m_state_idx_x != common::kNullIndex && m_state_idx_x < scene.car_state_values.size()) render_state.x = scene.car_state_values[m_state_idx_x];
  if (m_state_idx_y != common::kNullIndex && m_state_idx_y < scene.car_state_values.size()) render_state.y = scene.car_state_values[m_state_idx_y];
  if (m_state_idx_yaw != common::kNullIndex && m_state_idx_yaw < scene.car_state_values.size()) render_state.yaw = scene.car_state_values[m_state_idx_yaw];
  
  m_viewport_panel.handleInput(m_window, render_state);

  static bool pause_key_was_pressed = false;
  bool pause_key_pressed = glfwGetKey(m_window, gKeyBindings.pauseSimulation) == GLFW_PRESS;
  ...
  bool reset_key_pressed = glfwGetKey(m_window, gKeyBindings.resetSimulation) == GLFW_PRESS;
  ...
  if (m_gui_control_source) {
    const auto* desc = m_simulator.getCurrentModelDescriptor();
        ...
        bool key_w = glfwGetKey(m_window, gKeyBindings.carAccelerate) == GLFW_PRESS;
        bool key_s = glfwGetKey(m_window, gKeyBindings.carBrake) == GLFW_PRESS;
        bool key_a = glfwGetKey(m_window, gKeyBindings.carSteerLeft) == GLFW_PRESS;
        bool key_d = glfwGetKey(m_window, gKeyBindings.carSteerRight) == GLFW_PRESS;
        
        bool steering_rate_mode = false;
        if (m_setting_idx_steering_mode != common::kNullIndex && desc->setting_values) {
            steering_rate_mode = desc->setting_values[m_setting_idx_steering_mode] == 1;
        }
        ...
        if (m_input_idx_ax != common::kNullIndex) {
            ...
            if (key_w) input[m_input_idx_ax] = max_ax;
            else if (key_s) input[m_input_idx_ax] = min_ax;
        }
        ...
        if (steering_rate_mode) {
            if (m_input_idx_wheel_rate != common::kNullIndex) {
                ...
                if (key_a) input[m_input_idx_wheel_rate] = max_rate;
                else if (key_d) input[m_input_idx_wheel_rate] = min_rate;
            }
        } else {
            if (m_input_idx_wheel_angle != common::kNullIndex) {
                ...
                if (key_a) input[m_input_idx_wheel_angle] = max_angle;
                else if (key_d) input[m_input_idx_wheel_angle] = min_angle;
            }
        }
...
void Application::setupPanels() {
    ...
    if (ImGui::BeginListBox("##MarkerList", ImVec2(-1, 200.0f))) {
      ...
      for (const auto& entry : markers) {
        ...
        if (current_ns != key.ns) {
          ...
          if (m_guiConfig.markerNamespaceVisibility.find(current_ns) == m_guiConfig.markerNamespaceVisibility.end()) {
        ...
        std::string label = "##marker_" + current_ns + "_" + std::to_string(key.id);
  ...
  m_right_panel.addSection("Simulation Control", [this]() {
    ...
    if (ImGui::SliderFloat("##SimRunSpeed", &run_speed_float, min_speed, max_speed, "%.1fx", ImGuiSliderFlags_AlwaysClamp)) {
    ...
    toggle_cfg.Flags = ImGuiToggleFlags_Animated | ImGuiToggleFlags_Bordered | ImGuiToggleFlags_Shadowed;
    ...
    if (ImGui::Toggle("##ControlMode", &async_toggle, toggle_cfg) && !control_disabled) {
    ...
    double control_period_input = m_simulator.getRequestedControlPeriodMilliseconds();
    double active_control_period = m_simulator.getControlPeriodMilliseconds();
    ImGui::SetNextItemWidth(-1);
        if (ImGui::InputDouble("##ControlPeriodMs", &control_period_input, 0.5, 1.0, "%.1f")) {
    ...
    double control_delay_input = m_simulator.getRequestedControlDelayMilliseconds();
    double active_control_delay = m_simulator.getControlDelayMilliseconds();
    ImGui::SetNextItemWidth(-1);
        if (ImGui::InputDouble("##ControlDelayMs", &control_delay_input, 0.5, 1.0, "%.1f")) {
  ...
  m_right_panel.addSection("Model Selection", [this]() {
      ...
      if (m_selected_model_index != common::kNullIndex && m_selected_model_index < m_availableModels.size()) {
      ...
      const float browse_width = ImGui::CalcTextSize("Browse").x + style.FramePadding.x * 2.0f + 10.0f;
      float first_row_width = ImGui::GetContentRegionAvail().x;
      float input_width = std::max(50.0f, first_row_width - browse_width - style.ItemSpacing.x);
      ImGui::SetNextItemWidth(input_width);
      ImGui::InputText("##paramProfilePath", m_param_file_buffer, sizeof(m_param_file_buffer));
      ImGui::SameLine();
      if (ImGui::Button("Browse##paramProfile", ImVec2(browse_width, 0.0f))) {
          IGFD::FileDialogConfig config;
          std::filesystem::path initial = m_param_file_buffer[0] ? std::filesystem::path(m_param_file_buffer).parent_path()
                                                               : std::filesystem::current_path();
      ...
      if (ImGui::Button("Load Profile", ImVec2(half_width, 0.0f))) {
          if (sendProfileCommand(::lilsim::AdminCommandType::LOAD_PARAM_PROFILE, std::string(m_param_file_buffer))) {
              syncParamProfileBuffer();
          }
      }
      ImGui::SameLine();
      ...
      if (ImGuiFileDialog::Instance()->Display("ParamProfileDialog", ImGuiWindowFlags_None, kFileDialogMinSize, kFileDialogMaxSize)) {
          if (ImGuiFileDialog::Instance()->IsOk()) {
              ...
              if (!file_path_name.empty()) {
                  std::strncpy(m_param_file_buffer, file_path_name.c_str(), sizeof(m_param_file_buffer) - 1);
                  m_param_file_buffer[sizeof(m_param_file_buffer) - 1] = '\0';
                  if (sendProfileCommand(::lilsim::AdminCommandType::LOAD_PARAM_PROFILE, file_path_name)) {
                      syncParamProfileBuffer();
                  }
              }
          }
  ...
  m_right_panel.addSection("Parameters", [this]() {
      const auto* desc = m_simulator.getCurrentModelDescriptor();
      if (!desc) return;
  ...
  m_right_panel.addSection("Settings", [this]() {
      const auto* desc = m_simulator.getCurrentModelDescriptor();
      if (!desc) return;
      ...
      for (size_t i=0; i<desc->num_settings; ++i) {
          ...
          if (!options.empty()) {
              ImGui::SetNextItemWidth(-1);
              if (ImGui::Combo(name, &v, options.data(), static_cast<int>(options.size()))) {
  ...
  m_right_panel.addSection("Track Loading", [this]() {
      ...
      if (ImGui::InputText("##trackdir", m_track_dir_buffer, sizeof(m_track_dir_buffer))) {
          scanTrackDirectory();
      }
      ImGui::SameLine();
      ...
      if (ImGuiFileDialog::Instance()->Display("TrackDirDialog", ImGuiWindowFlags_None, kFileDialogMinSize, kFileDialogMaxSize)) {
          if (ImGuiFileDialog::Instance()->IsOk()) {
              ...
              if (!selected_path.empty()) {
                  std::strncpy(m_track_dir_buffer, selected_path.c_str(), sizeof(m_track_dir_buffer) - 1);
                  m_track_dir_buffer[sizeof(m_track_dir_buffer) - 1] = '\0';
                  scanTrackDirectory();
              }
          }
      ...
      for(size_t i=0; i<m_availableTracks.size(); ++i) {
          ...
          if(ImGui::Selectable(m_availableTracks[i].c_str(), is_selected, ImGuiSelectableFlags_AllowDoubleClick)) {
              m_selected_track_index = i;
              if (ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left)) {
                  std::string filepath = std::string(m_track_dir_buffer) + "/" + m_availableTracks[i] + ".csv";
      ...
      if (ImGui::Button("Load Selected Track", ImVec2(-1, 30))) {
          if (m_selected_track_index != common::kNullIndex && m_selected_track_index < m_availableTracks.size()) {
              std::string filepath = std::string(m_track_dir_buffer) + "/" + m_availableTracks[m_selected_track_index] + ".csv";
              sendSetTrackCommand(filepath);
          }
      }
  });
  ...
  m_right_panel.addSection("Status", [this]() {
      ...
      if (m_state_idx_x != common::kNullIndex && m_state_idx_y != common::kNullIndex && m_state_idx_y < scene.car_state_values.size()) {
          double x = scene.car_state_values[m_state_idx_x];
          double y = scene.car_state_values[m_state_idx_y];
          ImGui::Text("Pos: (%.2f, %.2f)", x, y);
      }
      if (m_state_idx_v != common::kNullIndex && m_state_idx_v < scene.car_state_values.size()) {
...
void Application::render2D() {
  ...
  if (desc && !scene.car_state_values.empty()) {
      if (m_state_idx_x != common::kNullIndex && m_state_idx_x < scene.car_state_values.size()) rs.x = scene.car_state_values[m_state_idx_x];
      if (m_state_idx_y != common::kNullIndex && m_state_idx_y < scene.car_state_values.size()) rs.y = scene.car_state_values[m_state_idx_y];
      if (m_state_idx_yaw != common::kNullIndex && m_state_idx_yaw < scene.car_state_values.size()) rs.yaw = scene.car_state_values[m_state_idx_yaw];
      
      if (m_param_idx_wheelbase != common::kNullIndex &&
          desc->param_values &&
          m_param_idx_wheelbase < desc->num_params) {
          rs.wheelbase = desc->param_values[m_param_idx_wheelbase];
      }
      if (m_param_idx_track_width != common::kNullIndex &&
          desc->param_values &&
          m_param_idx_track_width < desc->num_params) {
          rs.track_width = desc->param_values[m_param_idx_track_width];
      }
      
      if (m_state_idx_ax != common::kNullIndex && m_state_idx_ax < scene.car_state_values.size()) {
          rs.ax = scene.car_state_values[m_state_idx_ax];
      }
      
      if (m_state_idx_steer_wheel_angle != common::kNullIndex && m_state_idx_steer_wheel_angle < scene.car_state_values.size()) {
          rs.steering_wheel_angle = scene.car_state_values[m_state_idx_steer_wheel_angle];
      }
      
      if (m_state_idx_steer_wheel_rate != common::kNullIndex && m_state_idx_steer_wheel_rate < scene.car_state_values.size()) {
          rs.steering_wheel_rate = scene.car_state_values[m_state_idx_steer_wheel_rate];
      }

      if (m_state_idx_wheel_fl != common::kNullIndex && m_state_idx_wheel_fl < scene.car_state_values.size()) {
          rs.wheel_fl_angle = scene.car_state_values[m_state_idx_wheel_fl];
      }

      if (m_state_idx_wheel_fr != common::kNullIndex && m_state_idx_wheel_fr < scene.car_state_values.size()) {
  ...
  float viewport_height = std::max(0.0f, static_cast<float>(m_height) - menu_bar_height);
  m_viewport_panel.draw(left_w, viewport_y, static_cast<float>(m_width) - left_w - right_w, viewport_height, rs);
...
bool Application::initZmqInterface() {
  ...
  try {
    m_admin_socket = std::make_unique<zmq::socket_t>(*m_zmq_context, zmq::socket_type::req);
    m_admin_socket->connect(comm::endpoints::ADMIN_REP_INPROC);

    m_control_pub = std::make_unique<zmq::socket_t>(*m_zmq_context, zmq::socket_type::pub);
    m_control_pub->connect(comm::endpoints::CONTROL_ASYNC_SUB_INPROC);

    m_metadata_sub = std::make_unique<zmq::socket_t>(*m_zmq_context, zmq::socket_type::sub);
...
bool Application::sendAdminCommand(::lilsim::AdminCommandType type,
  ...
  try {
    ...
    if (!reply.ParseFromArray(reply_msg.data(), static_cast<int>(reply_msg.size()))) {
...
bool Application::sendSetTrackCommand(const std::string& path) {
  ...
  bool success = sendAdminCommand(::lilsim::AdminCommandType::SET_TRACK,
                                  [&](::lilsim::AdminCommand& cmd) {
  ...
  if (success) {
    ...
    std::error_code ec;
    std::filesystem::path canonical = std::filesystem::weakly_canonical(track_path, ec);
    ...
    if (result.type == comm::MarkerSubscriber::MessageType::MarkerArray &&
      ...
      for (const auto& marker_proto : result.marker_array->markers()) {
        ...
        if (marker.type == MarkerType::CAR_SPRITE && marker_proto.has_car()) {
          ...
          car_data.wheelbase = car_proto.wheelbase() > 0.0 ? car_proto.wheelbase() : 1.0;
          car_data.track_width = car_proto.track_width() > 0.0 ? car_proto.track_width() : 1.0;
          ...
          if (car_proto.has_tint_opacity()) {
            car_data.tint_opacity = std::clamp(car_proto.tint_opacity(), 0.0, 1.0);
          }
          marker.car = car_data;
        ...
        m_marker_system.addMarker(marker_proto.ns(), marker_proto.id(), marker, sim_time);
      }
...
/**
 * @brief Initialize config paths, load the active config, and ensure fallback defaults exist.
 */
void Application::initializeGuiConfigSystem(const std::filesystem::path& install_root) {
  ...
  if (!loadGuiConfigFromDisk(active_path, loaded)) {
    spdlog::warn("[viz] Failed to load GUI config '{}'; falling back to default.", active_path.string());
    if (!loadGuiConfigFromDisk(m_defaultGuiConfigPath, loaded)) {
      spdlog::warn("[viz] Default GUI config '{}' missing or corrupted; regenerating.", m_defaultGuiConfigPath.string());
      loaded = GuiConfig{};
      if (!writeGuiConfigToDisk(m_defaultGuiConfigPath, loaded)) {
        spdlog::error("[viz] Unable to write default GUI config '{}'. Using in-memory defaults.", m_defaultGuiConfigPath.string());
      }
    }
...
/**
 * @brief Apply dimensional, visibility, and path state from the loaded GUI config.
 */
void Application::applyBasicGuiConfig() {
  ...
  if (!m_guiConfig.trackDirectory.empty()) {
    ...
    std::error_code ec;
    std::filesystem::path canonical = std::filesystem::weakly_canonical(dir_path, ec);
    std::string normalized = ec ? dir_path.string() : canonical.string();
    std::strncpy(m_track_dir_buffer, normalized.c_str(), sizeof(m_track_dir_buffer) - 1);
    m_track_dir_buffer[sizeof(m_track_dir_buffer) - 1] = '\0';
  ...
  if (!m_guiConfig.paramProfilePath.empty()) {
    std::strncpy(m_param_file_buffer, m_guiConfig.paramProfilePath.c_str(), sizeof(m_param_file_buffer) - 1);
    m_param_file_buffer[sizeof(m_param_file_buffer) - 1] = '\0';
  }
  ...
  m_marker_system.applyNamespaceVisibilitySnapshot(m_guiConfig.markerNamespaceVisibility);
...
/**
 * @brief Apply persisted input source and control mode selections via admin command.
 */
void Application::applyControlModeFromConfig() {
  ...
  if (!sendSetModeCommand(target_sync, !target_gui)) {
    spdlog::warn("[viz] Failed to apply input source/control mode from GUI config.");
    return;
  }
...
void Application::restoreModelFromConfig() {
  ...
  if (!fs::exists(model_path, ec)) {
    spdlog::warn("[viz] Configured model '{}' not found; keeping current model.", model_path.string());
    return;
  }
  if (m_simulator.loadModel(model_path.string())) {
    spdlog::info("[viz] Loaded model '{}' from GUI config.", model_path.string());
    onModelChanged();
...
/**
 * @brief Load the parameter profile defined in the GUI config via admin command.
 */
void Application::applyProfileFromConfig() {
  ...
  if (!std::filesystem::exists(m_guiConfig.paramProfilePath, ec)) {
    spdlog::warn("[viz] GUI config profile '{}' not found; ignoring.", m_guiConfig.paramProfilePath);
    return;
  }
  if (!sendProfileCommand(::lilsim::AdminCommandType::LOAD_PARAM_PROFILE, m_guiConfig.paramProfilePath)) {
    spdlog::warn("[viz] Failed to load parameter profile '{}' from config.", m_guiConfig.paramProfilePath);
  }
}
...
void Application::restoreTrackFromConfig() {
  ...
  if (!std::filesystem::exists(m_guiConfig.trackFile, ec)) {
    spdlog::warn("[viz] Configured track '{}' missing; skipping.", m_guiConfig.trackFile);
    return;
  }
 ...
 */
bool Application::loadGuiConfigFromDisk(const std::filesystem::path& path, GuiConfig& outConfig) {
  namespace fs = std::filesystem;
  if (path.empty()) {
    return false;
  }
  ...
  try {
    ...
    if (auto panels = root["panels"]) {
      if (auto left = panels["left"]) {
        ...
        cfg.leftPanelCollapsed = left["collapsed"].as<bool>(cfg.leftPanelCollapsed);
      }
      if (auto right = panels["right"]) {
        cfg.rightPanelWidth = right["width"].as<float>(cfg.rightPanelWidth);
        cfg.rightPanelCollapsed = right["collapsed"].as<bool>(cfg.rightPanelCollapsed);
      }
    }
    if (auto paths = root["paths"]) {
      cfg.modelPath = paths["model"].as<std::string>(cfg.modelPath);
      cfg.paramProfilePath = paths["param_profile"].as<std::string>(cfg.paramProfilePath);
      cfg.trackDirectory = paths["track_directory"].as<std::string>(cfg.trackDirectory);
    ...
    if (auto sim = root["simulation"]) {
      ...
      cfg.controlPeriodMs = sim["control_period_ms"].as<double>(cfg.controlPeriodMs);
      cfg.controlDelayMs = sim["control_delay_ms"].as<double>(cfg.controlDelayMs);
      if (auto source = sim["input_source"]) {
        std::string value = to_lower_copy(source.as<std::string>(cfg.guiInputSource ? "gui" : "client"));
      ...
      if (auto mode = sim["control_mode"]) {
        std::string value = to_lower_copy(mode.as<std::string>(cfg.syncControlMode ? "synchronous" : "asynchronous"));
    ...
    spdlog::warn("[viz] Unable to parse GUI config '{}': {}", path.string(), ex.what());
    return false;
  }
}
 ...
 */
bool Application::writeGuiConfigToDisk(const std::filesystem::path& path, const GuiConfig& cfg) {
  namespace fs = std::filesystem;
  try {
    ...
    spdlog::error("[viz] Failed to write GUI config '{}': {}", path.string(), ex.what());
    return false;
  }
}
...
bool Application::loadGuiConfigFromPath(const std::filesystem::path& path) {
  ...
  m_guiConfigDirty = false;
  refreshWindowTitle();
  spdlog::info("[viz] Loaded GUI config from '{}'.", m_activeGuiConfigPath.string());
  return true;
}
...
void Application::ensureDefaultGuiConfigExists() {
  ...
  if (!writeGuiConfigToDisk(m_defaultGuiConfigPath, defaults)) {
    spdlog::error("[viz] Failed to recreate default GUI config '{}'.", m_defaultGuiConfigPath.string());
  } else {
    spdlog::info("[viz] Regenerated default GUI config '{}'.", m_defaultGuiConfigPath.string());
  }
}

/**
 * @brief Determine the active config path by reading the pointer file if present.
 */
 ...
 */
void Application::ensureConfigPointerMatches(const std::filesystem::path& path) {
  if (m_configPointerPath.empty()) {
    return;
  }
  try {
    ...
    spdlog::warn("[viz] Failed to update GUI config pointer '{}': {}", m_configPointerPath.string(), ex.what());
  }
}
...
/**
 * @brief Track panel widths/collapse state and mark the config dirty when they change.
 */
...
void Application::processGuiConfigDialogs() {
  if (ImGuiFileDialog::Instance()->Display("GuiConfigSaveAs", ImGuiWindowFlags_None, kFileDialogMinSize, kFileDialogMaxSize)) {
    if (ImGuiFileDialog::Instance()->IsOk()) {
  ...
  if (ImGuiFileDialog::Instance()->Display("GuiConfigLoad", ImGuiWindowFlags_None, kFileDialogMinSize, kFileDialogMaxSize)) {
    if (ImGuiFileDialog::Instance()->IsOk()) {
      std::string path = ImGuiFileDialog::Instance()->GetFilePathName();
      if (!path.empty()) {
...
void Application::openGuiConfigSaveDialog() {
  std::filesystem::path start = m_activeGuiConfigPath.empty() ? m_defaultGuiConfigPath : m_activeGuiConfigPath;
  IGFD::FileDialogConfig config;
  config.path = start.parent_path().empty() ? "." : start.parent_path().string();
  config.filePathName = start.filename().string();
  config.countSelectionMax = 1;
  config.flags = ImGuiFileDialogFlags_Modal | ImGuiFileDialogFlags_ConfirmOverwrite;
  ImGuiFileDialog::Instance()->OpenDialog("GuiConfigSaveAs", "Save GUI Config", ".yaml", config);
}
...
void Application::openGuiConfigLoadDialog() {
  ...
  config.countSelectionMax = 1;
  config.flags = ImGuiFileDialogFlags_Modal | ImGuiFileDialogFlags_DontShowHiddenFiles;
  ImGuiFileDialog::Instance()->OpenDialog("GuiConfigLoad", "Load GUI Config", ".yaml", config);
}

bool Application::resetGuiConfigToDefault() {
  ensureDefaultGuiConfigExists();
  if (!loadGuiConfigFromPath(m_defaultGuiConfigPath)) {
    spdlog::warn("[viz] Failed to reset GUI config to default '{}'.", m_defaultGuiConfigPath.string());
    return false;
  }
  spdlog::info("[viz] GUI config reset to default '{}'.", m_defaultGuiConfigPath.string());
  return true;
}

/**
 * @brief Update the config's stored track directory from the current text buffer.
 */
...
void Application::syncParamProfileBuffer() {
  ...
  std::strncpy(m_param_file_buffer, source.c_str(), sizeof(m_param_file_buffer) - 1);
  m_param_file_buffer[sizeof(m_param_file_buffer) - 1] = '\0';
}
...
void Application::scanTrackDirectory() {
  ...
  namespace fs = std::filesystem;
  try {
    if (fs::exists(m_track_dir_buffer) && fs::is_directory(m_track_dir_buffer)) {
...
void Application::mainLoop() {
  ...
  if (delta_time < m_target_frame_time) {
    std::this_thread::sleep_for(std::chrono::duration<double>(m_target_frame_time - delta_time));
  ...
  if (m_surface_texture.status != WGPUSurfaceGetCurrentTextureStatus_SuccessOptimal && 
      m_surface_texture.status != WGPUSurfaceGetCurrentTextureStatus_SuccessSuboptimal) {
      if (m_surface_texture.texture) wgpuTextureRelease(m_surface_texture.texture);
      return; 
  }
  ...
  WGPUTextureView backbuffer = wgpuTextureCreateView(m_surface_texture.texture, &view_desc);
  ...
  color_attachment.clearValue = {m_clearColor[0], m_clearColor[1], m_clearColor[2], m_clearColor[3]};
