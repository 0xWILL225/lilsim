...
#define PARAM_LIST(X)                                                         \
  ...
  PARAM_DOUBLE_ENTRY(X, steering_rack_ratio, "steering_rack_ratio", 4.5, 0.0, \
                     15.0)

#undef SETTING_LIST
#define SETTING_LIST(SETTING, OPTION)                              \
  SETTING_ENUM_ENTRY(                                              \
      SETTING, OPTION, steering_input_mode, "steering_input_mode", \
      OPTION_ENTRY(OPTION, "angle") OPTION_ENTRY(OPTION, "rate"))
...
#define INPUT_LIST(X)                                                        \
  INPUT_DOUBLE_ENTRY(                                                        \
      X, steering_wheel_angle_input, "steering_wheel_angle_input",           \
      -steering_steering_wheel_angle_max, steering_steering_wheel_angle_max) \
  INPUT_DOUBLE_ENTRY(X, steering_wheel_rate_input,                           \
                     "steering_wheel_rate_input", -steering_wheel_rate_max,  \
                     steering_wheel_rate_max)                                \
  INPUT_DOUBLE_ENTRY(X, ax_input, "ax", -ax_max, ax_max)

#undef STATE_LIST
#define STATE_LIST(X)                                                   \
  STATE_DOUBLE_ENTRY(X, ax, "ax", -ax_max, ax_max)                      \
  STATE_DOUBLE_ENTRY(X, steering_wheel_angle, "steering_wheel_angle",   \
                     -steering_steering_wheel_angle_max,                \
                     steering_steering_wheel_angle_max)                 \
  STATE_DOUBLE_ENTRY(X, steering_wheel_rate, "steering_wheel_rate",     \
                     -steering_wheel_rate_max, steering_wheel_rate_max) \
...
/**
 * @brief Generic circular buffer based delay (in samples)
 */
struct DelayBuffer {
...
/**
 * @brief Steering dynamics with optional rate mode + delay
 */
struct SteeringDynamics {
  ...
  double step(double steering_wheel_angle_input,
              double steering_wheel_rate_input, double rack_ratio, double dt) {
    ...
    const double steering_wheel_angle_min =
        g_state_min[ST_steering_wheel_angle];
    const double steering_wheel_angle_max =
        g_state_max[ST_steering_wheel_angle];

    commanded_steering_wheel_angle =
        std::clamp(commanded_steering_wheel_angle, steering_wheel_angle_min,
    ...
    double filtered_steering_wheel_angle = front_wheel_angle * rack_ratio;
    steering_wheel_angle =
        std::clamp(filtered_steering_wheel_angle, steering_wheel_angle_min,
                   steering_wheel_angle_max);
    steering_wheel_rate =
...
/**
 * @brief Drivetrain dynamics with simple delay on ax (longitudinal
 * acceleration)
 */
struct DrivetrainDynamics {
  ...
  double step(double ax_input) { return delay.step(ax_input); }
};

/**
 * @brief Car model implementation
 */
struct CarModel {
  ...
  bool useSteeringRate() const {
    ...
    static_assert(S_steering_input_mode < S_COUNT,
                  "Missing setting: steering_input_mode.");
...
CarModel* car_model_create(double dt) {
  ...
  // initialize params, settings, inputs, states
...
void car_model_destroy(CarModel* model) { delete model; }

const CarModelDescriptor* car_model_get_descriptor(CarModel* model) {
  return &model->desc;
}

const char* car_model_get_name(void) { return "Kinematic Single Track"; }
...
void car_model_reset(CarModel* model, double dt) {
  ...
  model->steering_dyn.configure(dt, model->param_values[P_steering_delay],
                                use_rate);
...
void car_model_step(CarModel* model, double dt) {
  ...
  double front_wheel_angle =
      model->steering_dyn.step(wheel_angle_in, wheel_rate_in,
                               model->param_values[P_steering_rack_ratio], dt);
  ...
  double dyaw =
      (wheelbase > 0.0) ? (v / wheelbase) * std::tan(front_wheel_angle) : 0.0;
  double dv = ax;
